...
class WorkerSessionRegistry:
 ...
 def start(self,worker_id,workstream,telegram_chat_id,ttl=600):
  ...
  c='NVIDIA-CONTROL-BIND-'+secrets.token_urlsafe(24); exp=datetime.now(timezone.utc)+timedelta(seconds=ttl);self.db.execute('INSERT OR REPLACE INTO bind_challenges VALUES(?,?,?,?,0)',(worker_id,c,exp.isoformat(),telegram_chat_id));self.db.commit();return c
 def consume(self,worker_id,text,telegram_chat_id,session_key):
  ...
  if not r or r[3] or text!=r[0] or telegram_chat_id!=r[2] or datetime.fromisoformat(r[1])<datetime.now(timezone.utc):raise BindError('BIND_REJECTED')
