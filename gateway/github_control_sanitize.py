...
def _without_ascii_controls(text):
 return ''.join(c for c in text if not ord(c) < 32 and ord(c) != 127)
def sanitize_result(value):
 ...
 try:
  text=str(value) if not isinstance(value,(dict,list)) else json.dumps(value,sort_keys=True)
 except Exception:
  raise ValueError('UNSUPPORTED_RESULT') from None
 if _PRIVATE.search(_without_ascii_controls(text)):raise ValueError('PRIVATE_KEY_REJECTED')
